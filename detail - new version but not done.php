<?php   
    ...
    echo " this is the slug $slug <br> ";

    $article = $post->get_post("slug='$slug'");
 
    print_r($article);
 
    /** distrubute article data*/ 

    $file_name   = $article[0]['file_name'].'.jpg';
    $alt         = $article[0]['alt'];
    $_GET['id']  = $article[0]['table_id'];
    $_SESSION['table_id']   = $article[0]['table_id'];

    $modal['table_name']   =$article[0]['table_name'];
    $modal['table_id']   = $article[0]['table_id'];
 

  echo "<img alt='$alt' src='$mc->home_path/fs_folders/images/uploads/posted%20articles/detail/$file_name' /> ";
...
if (TRUE):   
    ...
    /** other function bellow */  

    echo " <div style='display:none' > ";   

		    if($_GET['id'] = helper::get_table_id(
		    	$_GET['id'],
		    	$_SESSION['table_id']))
	    	{
	    		if(!$_GET['id'])
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<!-- new plugin  -->
			<script type="text/javascript" src='<?php echo $mc->home_path; ?>/fs_folders/js/jquery-1.7.1.min.js'> </script>
			<script type="text/javascript" src='<?php echo $mc->home_path; ?>/fs_folders/js/function_js.js'></script>
		<!-- new plugin  -->
		...
		<?php   
 		...
 		?>   
		<!-- new home --> 
			<link rel="stylesheet" type="text/css" href="<?php echo $mc->home_path; ?>/fs_folders/index_home/home_css/home.css">
		<!-- end home --> 
		<!-- new lookdetails -->

			<script type="text/javascript" src='<?php echo $mc->home_path; ?>/fs_folders/fs_lookdetails/lookdetails_js/lookdetails_ajax.js'></script>
			<script type="text/javascript" src='<?php echo $mc->home_path; ?>/fs_folders/fs_lookdetails/lookdetails_js/lookdetails_query.js'></script>
			<link rel="stylesheet" type="text/css" href='<?php echo $mc->home_path; ?>/fs_folders/fs_lookdetails/lookdetails_style/lookdetails.css'>
	...
	<body onload="modal_thumbnail( 'fs_postedarticles' , '<?php echo $modal['table_id']; ?>' , 'small' , 'modal-thumbnail-loader' , 'load-thumbnail' , 'modal-detail' )"  itemscope itemtype="http://schema.org/Product"   > 
		<?php  
			...
			include_once("$mc->home_path/fs_folders/google_analytics/analyticstracking.php");      
								...
								// require("fs_folders/API/fb.php");  
				 				...
				 				?>
				 					<!-- new comment -->
										...
										<link rel="stylesheet" type="text/css" href="<?php echo $mc->home_path; ?>/fs_folders/fs_lookdetails/style1.css">
										<link rel="stylesheet" type="text/css" href="<?php echo $mc->home_path; ?>/fs_folders/fs_lookdetails/look_comment_items/style.css"> 
									<!-- end comment -->   
				 			</div>  
															...
															<td id="ads-banner" >  
																<center>
																	<a href="r?loc=http://theschoolofstyle com/" target="_blank" >
																		<img id='a-b' src="<?php echo $mc->home_path; ?>/fs_folders/images/banner/new-sos-banner1.png" >
																	</a>
																</center> 
															</td>
														<tr> 
																...
																<center> 
																	<div id="load_look_picture_and_tags"  >
																		...
																		<?php $article_modal_style = 'margin-top: -60px;width:889px'; ?>
					      												...
					      												<?php  else: ?>  
					      													<iframe src= "<?php echo $modal['source_item']; ?>?autoplay=1&showinfo=0&rel=0"  style='width:100%; height:390px; margin-top:-60px; border-radius:5px 5px 0px 0px' frameborder='0'  webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
					      												<?php 
															 		...
															 		<center>
																 		<table id="lfdc_t2" style="margin-top:20px;   " border="0" cellpadding="0" cellspacing="0"  > 
																 			<tr>   
																 				...
																 				<td id='ld_eyes' >  
																	 				<img src="<?php echo $mc->home_path; ?>/<?php echo $mc->path_icons;?>/Views-Icon.png" title="views" > 
																 				</td>
																 				<td> 
																 				...
																 				<td id='total_arrow' > 	 
																 					...
																 					<img src="<?php echo $mc->home_path; ?>/<?php echo "$mc->genImgs/$modal[src_img_drip]"; ?>"  title="drips"   class="modal-drip-img<?php echo $modal['table_id'] ?>"   onclick="drip_popup_show( '<?php echo $mno; ?>' , '<?php echo $modal['table_name'] ?>' , '<?php echo $modal['table_id']; ?>' , '<?php echo $modal['title']; ?>' , 'Article' , '<?php echo $modal['mno'] ; ?>' , '<?php echo ".modal-drip-img$modal[table_id]"; ?>' , '<?php echo "$mc->genImgs/look-icon-drip-selected.png"; ?>' , '<?php echo $modal['table_id']; ?>' , '#stat-look-dripted<?php echo $modal['table_id']; ?>'  )" >  
																 				</td>																																																						  
																	 			<td > 																																  
																 				...
																 				<td id='ld_square_with_arrow'  > 
																 					<img src="<?php echo $mc->home_path; ?>/<?php echo $mc->path_icons;?>/share-icon.png" title="share"   onmouseover="share_mouser_over('<?php echo $modal['table_id']; ?>')" onmouseout="share_mouser_out('<?php echo $modal['table_id']; ?>')" >
																 				...
																 				<?php  if (  $modal['type'] == 'image'): ?> 
																	 				<td id='ld_scope' >
																	 					<a href="z?i=<?php echo $modal['table_id']; ?>&tn=<?php echo $modal['table_name']; ?>"  target="_blank" >
																	 						<img src="<?php echo $mc->home_path; ?>/<?php echo $mc->path_icons;?>/zoom-icon.png" title="zoom" >
																	 					</a> 	
																	 				</td>  
																 				<?php endif; ?>     
																 				<td id="ld_padding" >  
																 				...
																 				<td id="ld_hide_show" > 
																 					<img style="display:none"  id="rectangle-image-footer-hide" title="hide" onclick="hide_look_foooter_rectangle()"   src="<?php echo $mc->home_path; ?>/<?php  echo  "$mc->path_icons/hide-icon.png"; ?>" /> 
																 				</td> 
																 			<tr> 
																 				<td>
																 				</td><td>
