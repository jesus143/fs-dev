...
s_a[252]="Bulawayo|Harare|ManicalandMashonaland Central|Mashonaland East|Mashonaland West|Masvingo|Matabeleland North|Matabeleland South|Midlands";
	...
	function print_state( state_id, state_index ) {   
		// alert("country selected and select state now.")
        var option_str = document.getElementById(state_id);
        option_str.length=0; // Fixed by Julian Woods
        option_str.options[0] = new Option('State / Providence','State / Providence');
        option_str.selectedIndex = 0;
	...
	function print_city ( city_id, city_index ) {
		 // alert(" id = "+city_id+" , province index = "+city_index);
		var option_str = document.getElementById(city_id);
        option_str.length=0; // Fixed by Julian Woods
        option_str.options[0] = new Option('City','');
        option_str.selectedIndex = 0;
        var state_arr = s_a[city_index].split("|");
        for (var i=0; i<state_arr.length; i++) {
                option_str.options[option_str.length] = new Option(state_arr[i],state_arr[i]);
    	}
	}
	function print_zipcode ( ) {
		// body...
	}
...
$xmlcountries = 'http://ws.geonames.org/countryInfo&#039';
