...
function lookdetails_loaded (mno, style, table_id, table_name) {
    ...
    lookdetails_thumbnail(
        "lookdetails_thumbnail" ,
        "fs_folders/modals/lookdetails/lookdetails_modals.php?load=load_look_thumbnail&mno="+mno+'&style='+style+'&table_id='+table_id+'&table_name='+table_name,
        "load"
    );
...
/**
 * load the details modal
 */
function load_details_feed_modal() {
...
function ajax_insert_and_append_result( view_result_id , data , loader ){
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            // console.log('print comment now' + xmlhttp.responseText);
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            for(i=1;i<r.length;i++){
                c++;
                // alert(r[i]);
                $('#home_res_thumbnails'+c).append("<li >"+r[i]);
                if (c >  14  ) {
                    c=0;
                }
            }
            ...
            //future function need to transfer a callback function 
...
function  look_show_hide_tags( mno ) {
...
function get_login_stat ( ) {
    var mno = $('#print_login_stat_id').text(); 
        ...
        if(mno == 136) {      
          ...
          //  alert('you are login and you can do anything');
        } 

 
    /* 
     * This feature is to auto show the log in and auto scroll up the page till the header 
     * 
        if(mno == 136) {     
            // show the login area 
            show('#login-wrapper');    
            //alert('show the login area'); 
            fs( 'arrow-up' );
            throw new Error('This is not an error. This is just to abort javascript'); 
        } else {   
            alert('you are login and you can do anything');
...
function follow_unfollow_fs_member (buttonid , mno1 , page, counterId) {
    ...
    if(page != 'feed') {
        ...
        if($(buttonid).attr('status') == 'follow') {
            ...
            $(buttonid).attr('status', 'following');

            auto_count(counterId);

        } else {

            $(buttonid).attr('status', 'follow');

            auto_count_subtract(counterId);
    ...
    if ( isLogin == true ) {
        ...
        if (page == 'profile'){
            ...
            var button_following = 'fs_folders/images/profile/following.png';
...
function look_comment_thump_up_or_down (plcno, action , id, thumbsName, plno, trateid, crated, table_name, rate_type, mno)
{
    ...
    /**
     * for mouse over
     * remove the id of the like comment
     */
    // $(id).attr('id', 'liked');
}
...
function auto_count_subtract( id ) {
    var trate = parseInt( $(id).text() ) - 1;
    // alert( trate );
    $(id).text( trate );
}
...
function look_comment_number_clicked( number , table_id , type , mno , tab , table_name ,  action , process , page ) {
    ...
    switch ( type ) {
        case 'rate-look':
            ...
            append_home(
                'rate-look-res',
                'fs_folders/modals/rate-look/rate-look-modals.php?pagenum='+number+
                '&rate_style='+rate_style+
                '&rate_looks='+rate_looks+
                '&rate_status='+rate_status+
                '&rate_gender='+rate_gender+
                '&rate_plus_blogger='+rate_plus_blogger+
                '&rate_latest='+rate_latest+
                '&rate_table_name='+rate_table_name+
            ...
            //ajax_send_data(
            //    '',
            //    'fs_folders/modals/rate-look/rate-look-modals.php?retrievedas=rate-next-prev'+
            //    '&rate_style='+rate_style+
            //    '&rate_gender='+rate_gender+
            //    '&rate_plus_blogger='+rate_plus_blogger+
            //    '&rate_latest='+rate_latest+
 ...
 * @evente
 * @var type
 * This is use for submitting a comment to the database
...
function comment_box_hit_enter_js( plno , e , type, rCDiv) {
    ...
    if (e.keyCode == 13 || type == 'post-botton' ) {
        ...
        //if div is empty should append the default response container div
        // if(rCDiv == '') {
        //     rCDiv = 'comments_result'; 
        // } 
...
/**
 * @var idClass
 * This is to clean the text or value.
 * usual this what we call reset or clear
 */
function clear_input(idClass) {
    $(idClass).val('');
    $(idClass).text('');
}
        ...
        case 'search-field':
            ...
            setTimeout(function(){
                ...
                if ( header['keySearch'] == '' ) {
                    ...
                    var data = 'action=header&type=member_suggest&keySearch='+header['keySearch'];
                    ajax_send_data(
                        header['result'],
                        'fs_folders/modals/general_modals/gen.modals.func.php?'+data,
...
function get_rate_look_sort ( ) {
    ...
    rate_looks  = $( '#rate-look-latest'  ).val();  //top looks , all looks
    //rate_status = $( '#rate-look-status' ).val();  // rated or un rated
    //

    rate_style        = $( '#rate-look-style'  ).val();
    rate_gender       = $( '#rate-look-gender'  ).val();
    rate_plus_blogger = $( '#rate-look-plus-blogger' ).val();
    rate_latest       = $( '#rate-look-latest'  ).val();
    rate_status       = $( '#rate-look-status'  ).val();
    rate_table_name   = $( '#rate-table-name'  ).val();
    rate_look_topic   = $( '#rate-look-topic'  ).val();
...
function rate_look_retrieve_nextprev_and_look_modals ( ) {
    ...
    // change_url (url); 
    // alert('page loaded rate_look_topic = ' + rate_look_topic);
    ajax_send_data(
        '',
        'fs_folders/modals/rate-look/rate-look-modals.php?retrievedas=rate-next-prev'+
        '&rate_style='+rate_style+
...
function fs_popup( page , action , process , method , table_id , table_name , type , title ) {
    ...
    switch(  page ) {
        ...
        case 'details-popup-continue-reading':
            ...
            $( "#popup-response" ).load( "lookdetails-popup.php?table_name="+table_name+'&table_id='+table_id, function() {
                $('#popup-more-doing-the-action-loader').css('display', 'none');
            });
            break;
...
function modal_comment_like_dislike( mno , table_id , table_name , rate_type , thumbsName , thumbs_id , id , trateid, name) {
    ...
    // remove id of the thumbsup to stop mouse over again changing image
    ...
    if ( cstat == 'not rated comment' ) {
        ...
        if($(trateid).text() == '') {
           ...
           // alert('add name = ' + name + ' id = ' + trateid+'-name' + ' stat = ' +$(trateid+'-stat').text()); 
            if($(trateid+'-stat').text() == '1') { 
                $(trateid+'-name').prepend(name + ' and '); 
            } else { 
                $(trateid+'-name').prepend(name);
            }  
        } else {
            //alert('add number');
            $(trateid).text(trate);
...
function showHideParentOverFlow(parentOverFlowHide, status) {
...
function member_suggest_load( process , limit_start , limit_end , mno, response_container, type, loader) {
    showHideParentOverFlow('body', 'hide');

    // load suggested member from the site

    setTimeout(function(){
        member_suggest ( process , limit_start , limit_end , mno, response_container, type, loader);
    }, 1000);

    // load suggested member joined fb from fb friends
    setTimeout(function(){
        process = 'view-more-notification';
        limit_end = 5;
        limit_start = 0;
        response_container = 'fb-friends-on-fs-container-td';
        type = 'fb-friends-on-fs';
        loader = 'notification-view-more-loader-2';
        $("#"+response_container).text('');
        member_suggest ( process , limit_start , limit_end , mno, response_container, type, loader);
    }, 2000);


    //load facebook friends to invite  
    setTimeout(function(){
        process = 'view-more-notification';
        limit_start = 0
        limit_end = 5
        response_container = 'fb-friends-to-invite-on-fs-container-td';
        type = 'fb-friends-to-invite-on-fs';
        loader = 'notification-view-more-loader-3';
        $("#"+response_container).text('');
        member_suggest ( process , limit_start , limit_end , mno, response_container, type, loader);
    }, 3000);
...
function member_suggest ( process , limit_start , limit_end , mno, response_container, type, loader) {
    ...
    if ( send_notification == true ) {
        ...
        var data1 = 'action=notification&process='+process+'&limit_start='+limit_start+'&limit_end='+limit_end+'&mno='+mno+'&type='+type;
        var view_result_id = 'fs-general-ajax-response';
        var data = 'fs_folders/modals/general_modals/gen.modals.func.php?'+data1;
        var loader = loader;
        ...
        switch ( process ) {
            case 'view-more-notification':
                ...
                // loader = 'notification-view-more-loader';
                // view_result_id = 'member-suggest-container-td';
                $('#'+loader).css('visibility','visible');
                ...
                xmlhttp.onreadystatechange = function() {
                    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                        ...
                        $("#"+response_container).append(xmlhttp.responseText);

                        $('#'+loader).css('visibility','hidden');

                        // document.getElementById('fs-general-ajax-response').innerHTML = xmlhttp.responseText;
                        // document.getElementById(response_container).innerHTML = xmlhttp.responseText; 
                    }
                }
                xmlhttp.open('GET',data,true);
                xmlhttp.send();
            ...
            case 'view-notification-load':
                ...
                xmlhttp.onreadystatechange = function() {
                    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                        ...
                        //alert(xmlhttp.responseText);
                        document.getElementById(response_container).innerHTML = xmlhttp.responseText;
                        // document.getElementById('member-suggest-container-td').innerHTML = xmlhttp.responseText;
                        $('#'+loader).css('visibility','hidden');
                    }
                }
                xmlhttp.open('GET',data,true);
...
function postarticle_select_category(dataContainer, dropDownId) {
...
function modal_thumbnail ( table_name , table_id , size , loader , process , action ) {
    ...
    // conslog.log(' modal_thumbnail ( '  + table_name + ' = table_name '   + table_id + ' = table_id '  + size + ' = size '   + loader + ' = loader '  + process + ' = process '  + action + ' = action ' );
    ...
    if ( process == 'load-thumbnail' ) {
        ...
        // );     
        /**
         * This is the old version that change the content when thumbnails clicked
         */
        /* 
         print_new_look(
...
function modal_detail( table_name , table_id , pagenum , action , process , orderby )  {
    ...
    if ( process == 'load-next-comment-page' ) {
        ...
        ajax_send_data(
            'comments_result',
            // 'comment_content',  <-- this is the result sort for the old version
            'fs_folders/modals/general_modals/gen.modals.func.php?'+data,
            'modal-comment-loader-test1 , modal-comment-loader-test2'
        );
    }
 ...
 * @param btn_id
 */
function welcome_thumbnail_select(idName, id, new_id, btn_id, type) {
...
function welcome_about(errorDiv,fieldId,tab) {
    ...
    $.get("fs_folders/modals/welcome/welcome_modals.php?tab="+tab+"&typed="+typed, function(data, status){
        ...
        if(data.indexOf('error') > 0){
          //  alert('error');
            ...
            $('#'+errorDiv+'-validate').val('exist');

        } else  {
           // alert('not error');
           // $('.next-about').fadeIn('slow');
            $('#'+errorDiv+'-validate').val('');
        }
...
function welcome_select_brand_tab(brand, num, type) {
    ...
    if(type == 'topic') {
        loader   = '#welcome-loader-topic';
        tab      = 'topic';
        tbrand   = 20;
        page     = 1;
        divModal = 'welcome-container-ul-row2';
        divMore  = 'welcome-popup-brand2';
        url      = "fs_folders/modals/welcome/sort_brand.php?brand="+brand+"&page="+page+'&type='+type;
        fieldId  = 'welcome-topic-field';
        idName   = '#img-thumbnail-hover-row2-';  
        select   = 'selected_2';
        btn_id   = '#welcome-show-more-row2';
    }
    else {
        loader   = '#welcome-loader-brand';
        gender   = $('#gender').val();
        tab      = 'brand';
        tbrand   = 20;
        page     = 1;
        divModal = 'welcome-container-ul-row1';
        divMore  = 'welcome-popup-brand1';
        url      = "fs_folders/modals/welcome/sort_brand.php?brand="+brand+"&page="+page+'&type='+type+'&gender='+gender
        fieldId  = 'welcome-brand-field';
        idName   = '#img-thumbnail-hover-row1-'; 
        select   = 'selected_1'; 
        btn_id   = '#welcome-show-more-row1';
    }
 
    //change tab 
    for (var i = 1; i < tbrand; i++) {
        if(num == i)  {
            $('#'+tab+'-tab-'+i).attr('class','active');
        }
        else {
    ...
    $.get(url, function(data, status) {
        ...
        /*
        * update the brand to selected that is selected already 
        * This is usually happend when changing the tabs 
        */
        var selected = $('#'+fieldId).val(); 
        var data1 = selected.split('-');   

        for(var i = 1;  i < data1.length; i++ ) {
            id = data1[i];
            welcome_thumbnail_hover(idName, id);
            welcome_thumbnail_select(idName, id, select, btn_id, 'auto');  
        } 
...
function validate_about() {
    ...
    var error_username = $('#username-error-validate').val();
    var error_bname    = $('#bname-error-validate').val();
    var error_burl     = $('#burl-error-validate').val();
...
function saveAjax (category, containerId, page) {

    if(page == 'account') {
        document.location = 'account?at=7&updated='+category;
    } else {
...
function hide_show_hide(hideId, showId) {
