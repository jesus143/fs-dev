<?php
	...
	function delete_w_2($table_name,$where_array,$oparators) {
 	...
 	function update1($tableName,$rowname,$valuename,$whereArray=null) {
 		...
 		// echo " update1($tableName,$rowname,$valuename ) ";
 		// print_r( $whereArray);
	...
	function insert($table_name,$row_name_array,$values_array,$row_id_name=null) {
		for ($i=0; $i < count($row_name_array) ; $i++) { 
			if ($i<1) {
				...
				$b0=mysql_query("INSERT INTO  $table_name ($row_name_array[$i]) value('$value') ");
 	...
 	function update_v1( $val=array() , $where=array() ) { 
 			

 		// table name of the row is not allowed to update 
		...
		foreach ($val as $key => $value ) {  
			...
			if ( $key  != 'table_name' ) { 
	    		if (is_string($value)) {
	    			$res=mysql_query("UPDATE $table_name SET $key = '$value' WHERE $w ");  
	    		}
	    		else{
	    			$res=mysql_query("UPDATE $table_name SET $key = $value WHERE $w "); 
	    		}  
    			...
    			// echo "not allowed to update tablename <br>";
    		}
    	} 
    	return $res;
	}
 	function insert_v1( $val=array() ) { 
		$c=0; 
		$table_name  = $val['table_name'];
		$row_id_name = $val['row_id_name']; 
		foreach ($val as $key => $value ) {  
    		if ( $c==0 ) {
    			$b0=mysql_query("INSERT INTO  $table_name ($key) value('$value') ");
    			$last_id = mysql_insert_id(); 
    		}
    		else{
    			$b1=mysql_query("UPDATE $table_name SET $key = '$value' WHERE $row_id_name = $last_id");
    		} 
    		$c++; 
    	} 
    	return $last_id; 
	...
	function select_v3( $tablename , $selectwhat , $w ) { 
		   // echo "  $tablename , $selectwhat , $w"; 
		$c=0;
		$result = array( );
		$mno = 133;  

		$Q = mysql_query(" SELECT $selectwhat FROM  $tablename  WHERE $w ");
	...
	function select_v4( $array ) {


		$db['tablename'] = ( !empty($array['tablename'])) ? $array['tablename'] : null ;
		$db['rows']      = ( !empty($array['rows']))      ? $array['rows']      : null ;
		$db['order']     = ( !empty($array['order']))     ? $array['order']     : null ;
		$db['limit']     = ( !empty($array['limit']))     ? $array['limit']     : null ;
		$db['like']      = ( !empty($array['like']))      ? $array['like']      : null ;
		$db['where']     = ( !empty($array['where']))     ? $array['where']     : null ;
		$db['join']      = ( !empty($array['join']))      ? $array['join']      : null ;
		$db['countas']   = ( !empty($array['countas']))   ? $array['countas']      : null ;
		$db['type']      = ( !empty($array['type']))      ? $array['type']      : null ;
		$db['response']  = null;
		$db['result']    = null; 
		$db['counter']   = 0; 
		...
		# select 
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'*',  'where'=>"table_id > 133 and table_id < 140 ", 'order'=>'sno desc',  'limit'=>'0 , 2' ) );  
		# count
		# $response = select_v4( array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'count(*) as sno',   'order'=>'sno desc'    ) );
		# AVG
		# $response = select_v4( array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'AVG(sno)',   'order'=>'sno desc'    ) );
		# SUM
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'SUM(sno)',   'order'=>'sno desc'    ) );
		# MAX
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'MAX(sno)',   'order'=>'sno desc'    ) );
		# MIN
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'MIN(sno)',   'order'=>'sno desc'    ) );
		# FIRST
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'sno',        'order'=>'sno asc limit 1'   ) );
		# LAST
		# $response = select_v4(  array( 'type'=>'select',  'tablename'=>'fs_search', 'rows'=>'sno',        'order'=>'sno desc limit 1'  ) );


 		$q = 'SELECT '.$db['rows'].' FROM '.$db['tablename'];  
		...
		$query = @mysql_query($q);   
		if (!empty($query)) {
			while ($db['result']=mysql_fetch_array($query)) {  		
				$db['response'][$db['counter']] = $db['result'];
				$db['counter']++;
			}
			return $db['response'];
		}else { 
	...
	function make_links_clickable1($text){
