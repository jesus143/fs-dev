<?php
	...
	// if($_SESSION['mno']=="")
	 ...
	 check_if_look_posted();
	 detect_attempt_to_hacker();
	 
	 
	 // echo "This is the id = $_GET[id] <br>";
...
<script type="text/javascript" src="js1/jQv1.8.2.js"></script>
...
<script type="text/javascript">
	...
	$(window).ready(function(){
		...
		function specific_showhide(){ 
		   $('.parent_option').click(function(){
		   		// alert('you clicked the parent_option');
		   		var id =$(this).attr('id');
				if ($('#s'+id).attr('class') == 'hide') {
					$('#s'+id).fadeIn('fast');
					$('#s'+id).removeClass('hide').addClass('show');
				}else{
					$('#s'+id).removeClass('show').addClass('hide');
					$('#s'+id).fadeOut('fast');
				}
		   } );

		   $('.rspam').click(function(){
		   		var id =$(this).attr('id');
		   		$('#s'+id).removeClass('show').addClass('hide');
				$('#s'+id).fadeOut('fast');
		   } );
...
/*	border: 1px solid #000;
	...
	<script>
			...
			function addasfriend(mno1,mno2)
			{
				xmlhttp.onreadystatechange=function()
				{
					if (xmlhttp.readyState==4 && xmlhttp.status==200)
					{	
						...
						{
							getID("frnd").innerHTML="<i>Friend request sent</i>";
						}
						else
						{	
							//alert(xmlhttp.responseText);		
				...
				xmlhttp.open("GET","ajaxquery/addasfriend.php?mno1="+mno1+"&mno2="+mno2,true);
				xmlhttp.send();
			}
		...
		function highlight(id)
		{
			normal();
			for(i=1;i<=id;i++){
				getID("r"+i).src="images/rate.png";
			}
		}
		function normal(){
			for(i=1;i<=5;i++){
				getID("r"+i).src="images/rateover.png";
			}
		}
	</script>
	...
	<br><Br>
		...
		<style>
			...
			#r5:hover,#r4:hover,#r3:hover,#r2:hover,#r1:hover{
				background:#ff860b;
			}
			
			.rQ,#r5,#r4,#r3,#r2,#r1,#r0,#r01{
			...
			.follow{
				width:200px; border:0; padding:8px;background:#9dc827;color:white;font:bold 12px arial;
			}
			.follow:hover{
				background:#148794;
			}
					...
					<?php
						...
						echo "<div style=\"padding:0 5px 5px 5px;\">
							<table width=100% style=\"border-collapse:collapse;padding:5px;font:15px helveticaBold;border-top:1px solid black;border-bottom:1px solid #aaa;\">
								<td width=*% id='td_lookName' ><a type=button style=\" text-decoration: none;foreground:black;border:0;background:none;font:bold 13px arial;cursor:pointer;padding:0\" href='http://fashionsponge.com/fs/rate'><font color='000000'>back to looks</font></a></td>
...
<!-- side content -->
				...
				<style type="text/css">
					...
					.adds td{ background-color: #dbdbdb; height: 125px; width: 125px; border: 1px solid #ccc; text-align: center;   }
							...
							if (!empty($r[0])) {
								...
								echo"
									<td valign=top align=left onclick=\"window.location='lookdetails.php?id=$r[0]'\" >
										<img title='$r[3]' src='images/members/posted looks/$r[0].jpg' style='width:137px; height:137px' />
									</td> 
								";
				...
				<form method=post>
					<?php
						...
						while($plr=mysql_fetch_array($pl)){
							if ($plr[0]!=133 and  $plr[0]!=134 and $plr[0]!=135 and $plr[0]!=136 ) {
							 
								// echo "mno1=$_SESSION[mno]  and mno2= $plr[0] ";
		...
		<!-- end user if is log on -->



	<!-- new user if not log on -->
			<?php }else{ ?>
			<?php set_redirect_to_lookdetails_if_not_login(); ?>
			<?php require("lookdetail_notlogin.php");  ?>
			<?php require("footer.php"); ?>
			<?php }?>		
			<!-- end user if not log on -->	
			</div>
		</div>
	</body>
</html>
...
<?php 
		 	...
		 	function get_id_look_owner($lookid){
							$activity_posted_info=select_w_2('activity',6,array('action','Posted','_table_id',$lookid),'and');
							return $activity_posted_info;
						}
