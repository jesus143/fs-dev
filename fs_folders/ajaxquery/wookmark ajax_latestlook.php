			...
			<?php
				...
				$q = "SELECT * from activity act where act.action='posted' or act.action='joined' order by act.ano desc limit $from,$rows"; 
					...
					while($r=mysql_fetch_array($ex))
					{
						...
						if($r["action"]=="Posted" || $r["action"]=="posted" ){
								...
								<li >
									<div onmouseover=\"showRate($rsplq[0],'block')\" onmouseout=\"showRate($rsplq[0],'none')\" >
										<div style='position:absolute;display:none;' id='rate$rsplq[0]'>
											...
											<div style='position:absolute;'>
												<div style=\"overflow:hidden;font:bold 12px 'helvetica';padding:10px;width:150px;position:relative;left:50px;top:15px;background:url('images/trans-back.png');width:218px;color:white\">
						...
						else if($r["action"]=="Joined" || $r["action"]=="joined"){
							...
							echo"
								<li >
									<div  >";
										if($rsmemq["ispicset"]==1)
											echo"<a href='".$rsmemq["username"]."'><img src='images/members/".$rsmemq["mno"].".jpg' style='width:287px'  /></a><br>";
										else
											echo"<a href='".$rsmemq["username"]."'><img src='images/members/0.jpg' style='width:287px'  /></a><br>";
