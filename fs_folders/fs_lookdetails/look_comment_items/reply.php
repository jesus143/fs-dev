...
<?php
	...
	$repliesLen = reply_print( $mc , $st , $_SESSION['replies'] , $view_start , $view_stop , $plcno );
	...
	function reply_print( $mc , $st , $replies , $view_start , $view_stop , $plcno ) {  
	    ...
	    for ($j=$view_start; $j <$view_stop; $j++) { 
	    	...
	    	$sub_reply_start = true;
	    	$reply = $st->get_reply_of_reply($replies[$j]['plcr_no']);
	    	$reply_len = count($reply);
	    	$c = $j;
	    	// print_r($reply);
	    	// echo " start len of the reply = $reply_len plcr_no = ".$replies[$j]['plcr_no']."<br>";

	    	if ( $j < $repliesLen ) {
 
	    		for ($h=0; $h <= $reply_len; $h++) { 
 				$reply_allow = true; 
	    		if ($sub_reply_start) {
	    			// echo " allow reply true ";
	    			// main sub reply
	    			// main reply printed
	    			
	    			$_SESSION['plcr_no'] = $replies[$j]['plcr_no'];
	    			// echo " plcr_no = ".$_SESSION['plcr_no'].'<br>';
	  				$sub_reply_start = false;
	  				 
	  				$_SESSION['counter']++;	
	  				 

	  				// echo "c = $c main reply comment print <br>";
	  				;
		  			$start_ul_li = '<li>';
					$end_ul_li = '</li>';
				}  else { 
					// echo " allow reply false ";
					$reply_allow = false; 
					// sub reply printed

					if ( $_SESSION['plcr_no'] != $_SESSION['old_plcr_no'] ) {
						// echo "if  get all sub reply ".$_SESSION['plcr_no'].'<br> ';
						$_SESSION['sub_reply1'] = $st->get_reply_of_reply($_SESSION['plcr_no']);
						$_SESSION['old_plcr_no'] = $_SESSION['plcr_no'];
					}else { 
						// echo "<br> else  get all sub reply ".$_SESSION['plcr_no'].'<br> ';

					}	
					

					$replies = $_SESSION['sub_reply1']; 
					$c = $h;
					$c-=1;
					// echo "c = $c sub reply comment  total reply = ".count($reply)."<br>";
								...
								<td  > 
									<table border=0   id='img' class='img' > 
										<td>	  
											...
											<?php 
											 // $mc->member_thumbnail_display( $mc->ppic_thumbnail , $rmno , '../../../'.$mc->ppic_thumbnail  ); ?>
												...
												<td><span class='blue_bold' > ( </span> <span  id='gender' class='blue'  > <?php echo strtoupper($rmem_info['sub_gen']); ?>  RATED </span> 
													...
													<!--new user name replied --> 
														...
														<span class='blue_bold'> ( </span>  
									 					<span class='blue_bold' onmouseover="replied_name_hover(<?php echo "$replied_no"; ?>,<?php echo "$rplcno"; ?> )" onmouseout="replied_name_leaved(<?php echo "$replied_no"; ?>,<?php echo "$rplcno"; ?>  )"  style='cursor: pointer;' > replied to  <?php echo comment_repplied_name( $replied_no , $rplcno  ); ?>  </span> 
									 				 	<span class='blue_bold'> ) </span>
